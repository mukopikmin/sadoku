...
Deno.test("parses host and port options", () => {
  assertEquals(parseArgs(["README.md", "--host", "0.0.0.0", "--port", "4000"]), {
    file: "README.md",
    host: "0.0.0.0",
    port: 4000
  });
...
Deno.test("throws usage errors for invalid options", () => {
  assertInstanceOf(assertThrows(() => parseArgs(["README.md", "--port", "nope"])), CliUsageError);
  assertInstanceOf(assertThrows(() => parseArgs(["README.md", "--unknown"])), CliUsageError);
  assertInstanceOf(assertThrows(() => parseArgs(["a.md", "b.md"])), CliUsageError);
});
