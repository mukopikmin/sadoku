  ...
  try {
    ...
    try {
      ...
      const indexes = await database.execute<{ name: string }>(
        "SELECT name FROM sqlite_master WHERE type = 'index' AND name IN ('idx_comment_documents_file_path', 'idx_comments_document_id', 'idx_comments_document_start_line', 'idx_comment_replies_comment_id') ORDER BY name",
      );
      ...
      assertEquals(indexes.rows, [
        ...
        { name: "idx_comments_document_start_line" },
      ]);
      ...
      await database.execute(
        "INSERT INTO comments (document_id, local_id, start_line, end_line, original_start_line, original_end_line, body, created_at, updated_at) VALUES (1, 1, 2, 2, 2, 2, ?, ?, ?)",
