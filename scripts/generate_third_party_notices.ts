...
const render = (notices: Notice[]): string => {
  ...
  const summary = sorted.map((notice) =>
    `| ${notice.name} | ${notice.version} | ${notice.license} | ${notice.source} |`
  ).join("\n");
...
JavaScript assets. The following third-party components are included directly
or through bundled dependencies.
